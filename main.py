...
import configure
from vector_info import VectorInfo
...
def search_news():
    ...
    for search_str in search_strings:
        ...
        news_info = collect_naver(search_str, 1000)
...
def add_monitor():
    ...
    # Collect news from NAVER news.
    news_info = collect_naver(new_keyword, 1000)
    ...
    for i in range(len(group_info)):
        if i in selected_groups['group']:
            ...
            VectorInfo.append(nouns_info[i], 'selected')
        else:
            VectorInfo.append(nouns_info[i], 'banned')
